...
export default class PositionBar extends React.Component<IProps, {}> {
	...
	@observable
	lapTimeBestLap = INVALID;

	@observable
	lapTimeLastLap = INVALID;
	...
	private update = () => {
		...
		this.lapTimeBestLap = r3e.data.LapTimeBestSelf;
		this.lapTimeLastLap = r3e.data.LapTimePreviousSelf;
	};
	...
	private getUserIdFromSlot(slotId: number): number {
		...
		return r3e.data.DriverData[index].DriverInfo.UserId;
	}
				...
				{!this.props.relative &&
					this.props.settings.subSettings.lapTime.enabled && (
						<div className="lastLap">
							...
							<div className="label">{('Last time')}</div>
						</div>
					)}
